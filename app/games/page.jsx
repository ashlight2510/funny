...
export const metadata = {
  ...
  openGraph: {
    ...
    description: "주사위·포춘·메뉴 룰렛·선물·데이트 등 무료 게임·랜덤 뽑기 모음.",
    siteName: "FunnyFunny Cloud",
  },
  robots: { index: true, follow: true },
};

const FEATURED_GAME_SLUGS = ["random-food", "magic-8-ball", "fortune-cookie", "mood-random", "dice", "dinner-roulette"];
    ...
    <div className="bg-slate-50 text-slate-900 min-h-screen">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
        <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">
          무료 게임·랜덤 뽑기 모음 <span className="text-amber-600 font-bold">({tools.length}개)</span>
        </h1>
        <p className="mt-2 text-slate-600">
          주사위·포춘·메뉴 룰렛·선물·데이트·MBTI 등 한눈에.
        </p>
        <p className="mt-4 text-slate-600 text-sm max-w-2xl">
          동전 던지기·주사위·카드 뽑기, 포춘 쿠키·매직 8볼, 오늘 뭐 먹지·저녁 룰렛, 선물·데이트 아이디어,
          MBTI·별자리·타로·닉네임 생성 등 고민 해결·재미용 무료 게임·랜덤 뽑기만 모았습니다.
        </p>
        {featured.length > 0 && (
        ...
        <section className="mt-8">
          <h2 className="text-lg font-bold text-slate-800 mb-3">전체 게임·랜덤 뽑기 ({tools.length}개)</h2>
          <ul className="grid gap-3 sm:grid-cols-2">
            {tools.map((t) => {
              const slug = hrefToSlug(t.href);
              const path = getToolPath(slug);
